...
#Region "初期表示"
    ...
    Public Sub InitDisplay(tuban As String)
        ...
        Caller.lbl身長値.ForeColor = Color.Black
        Caller.lbl体重値.ForeColor = Color.Black

        Caller.txt腹囲.ForeColor = Color.Black
        Caller.txt身長.ForeColor = Color.Black
        Caller.txt体重.ForeColor = Color.Black
        Caller.txt腹囲少数.ForeColor = Color.Black
        Caller.txt身長少数.ForeColor = Color.Black
        Caller.txt体重少数.ForeColor = Color.Black
...
#Region "入力チェック"
    ...
    Public Function Validate() As Boolean
        ...
        If Caller.lbl体重値.Text.Equals(String.Empty) Then
            ...
            Return False
        End If
        If Caller.btn腹囲.Text = "腹囲実施しない" And Caller.lbl腹囲値.Text.Equals(String.Empty) And Caller.lbl腹囲.Enabled = True Then
            Caller.btnRegist.Enabled = True
        ...
        Dim db As DBAccess = New DBAccess
            ...
            '********************腹囲閾値***********************
            If Caller.btn腹囲.Text = "腹囲実施しない" And Caller.lbl腹囲.Enabled = True Then
    ...
    Public Sub Regist()
        Try
            If Validate() Then
                ...
                If Caller.btn腹囲.Text = "腹囲実施しない" And Caller.btnRegist.Enabled = True Then
                    datas.Clear()
...
#Region "受診者がいるか？"
    ...
    Public Function Load(caller As 身長体重) As Boolean
        ...
        Dac.ConnectInfo = caller.ConnectInfo
        If Not Dac.IsTtherePerson(AppSettings("HEIGHT")) Then
            Using msgwin As MessageWindow = New MessageWindow
                msgwin.IconImage = CommonConst.EXCLAMATION
...
#End Region
    ...
    Friend Sub btn腹囲_Click(realTenKey As Boolean)
        If Caller.btn腹囲.Text = "腹囲実施しない" Then
            Caller.lbl腹囲.Visible = False
            Caller.lbl腹囲単位.Visible = False
            Caller.lbl腹囲値.Visible = False
            Caller.txt腹囲.Visible = False
            Caller.txt腹囲少数.Visible = False
            Caller.Label5.Visible = False
            Caller.btn腹囲.Text = "腹囲実施する"
            Caller.txt体重少数.NextFocus = Caller.txt身長
            My.Settings.Is腹囲 = False
        Else
            Caller.lbl腹囲.Visible = True
            Caller.lbl腹囲単位.Visible = True
            If realTenKey Then
                Caller.txt腹囲.Visible = True
                Caller.txt腹囲少数.Visible = True
                Caller.Label5.Visible = True
                Caller.lbl腹囲値.Visible = False
            Else
                Caller.txt腹囲.Visible = False
                Caller.txt腹囲少数.Visible = False
                Caller.Label5.Visible = False
                Caller.lbl腹囲値.Visible = True

            End If
            Caller.btn腹囲.Text = "腹囲実施しない"
            Caller.txt体重少数.NextFocus = Caller.txt腹囲
            My.Settings.Is腹囲 = True
        End If
        Caller.txt身長.Focus()
    ...
    Friend Function IsExist(id As String) As Boolean
        ...
        Dim ret As Boolean
        If Dac.IsInspectionCode(id, AppSettings("HEIGHT")) OrElse Dac.IsInspectionCode(id, AppSettings("WEIGHT")) OrElse Dac.IsInspectionCode(id, AppSettings("GirthOfTheAbdomen")) Then
            Return True
        End If
        Return False
        'Caller.lbl腹囲値.Enabled = ret
        'Caller.lbl腹囲.Enabled = ret
        'Caller.lbl腹囲単位.Enabled = ret

        'Caller.txt腹囲.Enabled = ret
        'Caller.txt腹囲少数.Enabled = ret
        'Caller.Label5.Enabled = ret
        'If Not ret Then
        '    Caller.lbl腹囲値.Text = String.Empty
        '    Caller.txt腹囲.Text = String.Empty
        '    Caller.txt腹囲少数.Text = String.Empty
